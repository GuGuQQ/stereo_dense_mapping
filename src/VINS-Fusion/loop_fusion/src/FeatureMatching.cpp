...
featurematch::FeatureMatching::FeatureMatching( std::shared_ptr<DBoW3::Vocabulary> voc ):mVoc_(voc) {
    // 设置bow阈值条件
    mTh_ = 50;
    mNNRatio_ = 0.6;
}
...
featurematch::FeatureMatching::FeatureMatching( std::shared_ptr<DBoW3::Vocabulary> voc,
...
// 默认析构函数
featurematch::FeatureMatching::~FeatureMatching() {
    mVoc_ = nullptr;
}

// 利用暴力匹配法进行特征匹配
bool featurematch::FeatureMatching::MatchByBruteForce( cv::Mat& desp1, cv::Mat& desp2, std::vector<cv::DMatch>& matches ) {
...
// 利用基于词袋模型的方法进行特征匹配
bool featurematch::FeatureMatching::MatchByDBoW( cv::Mat& desp1, cv::Mat& desp2, std::vector<cv::DMatch>& matches ) {
...
// 利用投影法进行特征匹配
bool featurematch::FeatureMatching::MatchByProject( std::vector<cv::Point3d>& vp3d1, cv::Mat& desp1, 
...
// 设置bow匹配的阈值条件
void featurematch::FeatureMatching::SetThreshold( int threshold, float nn_ratio ) {
    mTh_ = threshold;
    mNNRatio_ = nn_ratio;
}

// 将描述子转换为bow向量
void featurematch::FeatureMatching::ComputeBoWVector( cv::Mat& desp, DBoW3::BowVector& bowVec, DBoW3::FeatureVector& featVec ) {
...
// 计算两个描述子间的匹配分数
int featurematch::FeatureMatching::ComputeMatchingScore( cv::Mat& desp1, cv::Mat& desp2 ) {
