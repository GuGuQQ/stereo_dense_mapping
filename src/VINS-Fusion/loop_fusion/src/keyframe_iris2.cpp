	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    reduceVector(matched_id, status);
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            putText(notation, "iris flag: " + to_string(old_kf->iris_flag) + " previous frame: " + to_string(old_kf->index) + "  sequence: " + to_string(old_kf->sequence), cv::Point2f(20 + COL + gap, 30), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);
