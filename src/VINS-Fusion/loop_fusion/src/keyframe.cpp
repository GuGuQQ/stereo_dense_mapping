...
KeyFrame::KeyFrame(double _time_stamp, int _index, Vector3d &_vio_T_w_i, Matrix3d &_vio_R_w_i, cv::Mat &_image,
		           vector<cv::Point3f> &_point_3d, vector<cv::Point2f> &_point_2d_uv, vector<cv::Point2f> &_point_2d_norm,
		           vector<double> &_point_id, int _sequence,int _iris_flag)
{
	time_stamp = _time_stamp;
	index = _index;
	vio_T_w_i = _vio_T_w_i;
...
*/   //   descriptor->compute(image, keypoints_orb, descriptors_orb);
     //    printf("得到ORB描述子");
...
bool KeyFrame::findConnection(KeyFrame* old_kf)
{
	...
	matched_2d_cur_norm = point_2d_norm;
       // cout<<"匹配点对为: "<<matched_2d_cur.size()<<endl;
	matched_id = point_id;
...
/*
            if(!old_img.empty()){
	        cv::imshow("loop connection",old_img);  
                }
	    cv::waitKey(100); 
*/
	    	...
	    	if (DEBUG_IMAGE)
	        {
	            ...
	            {
	                cv::Point2f cur_pt = matched_2d_cur[i];
	                cv::circle(loop_match_img, cur_pt, 5, cv::Scalar(0, 0, 255));
	            }
	            for(int i = 0; i< (int)matched_2d_old.size(); i++)
	            {
	                cv::Point2f old_pt = matched_2d_old[i];
	            ...
	            putText(notation, "Agent: " + to_string(old_kf->iris_flag) + " previous frame: " + to_string(old_kf->index) + "  sequence: " + to_string(old_kf->sequence), cv::Point2f(20 + COL + gap, 30), CV_FONT_HERSHEY_SIMPLEX, 1, cv::Scalar(255), 3);
	         ...
	         //   
				...
				//	cv::imwrite("loop connection.png",loop_match_img);  
	            	cv::waitKey(10);  
	...
	if ((int)matched_2d_cur.size() > MIN_LOOP_NUM)
	{
	    ...
	    relative_yaw = Utility::normalizeAngle(Utility::R2ypr(origin_vio_R).x() - Utility::R2ypr(PnP_R_old).x());
           // printf("偏航角为 %d ,相对位移为 %d \n", abs(relative_yaw),relative_t.norm());
	    //printf("PNP relative\n");
	  //  cout << "pnp relative_t " << relative_t.transpose() << endl;
	 //   cout << "pnp relative_yaw " << relative_yaw << endl;
	    if (abs(relative_yaw) < 30.0 && relative_t.norm() < 25.0)
	//    if (abs(relative_yaw) < 45.0 && relative_t.norm() < 50.0)
	    {
