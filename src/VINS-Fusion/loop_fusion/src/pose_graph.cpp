...
void PoseGraph::addKeyFrame(KeyFrame* cur_kf, int frame_index, bool flag_detect_loop)
{
    //shift to base frame
    Vector3d vio_P_cur;
    Matrix3d vio_R_cur;
   ...
   // cur_kf->index = frame_index;
    int loop_iris_flag=1;
    int old_iris_flag=-1;
   // loop_iris_flag = cur_kf->iris_flag;
    global_index++;
	int loop_index = -1;
    if (flag_detect_loop)
    {
        TicToc tmp_t;
/////////////////////////////////////By WJJ//////////////////////////////
  //      loop_index ,loop_iris_flag= detectLoop(cur_kf, cur_kf->index);
        loop_index = detectLoop(cur_kf, frame_index);
    }
    else
    {
        addKeyFrameIntoVoc(cur_kf);
       // printf("在addKeyFrame添加关键帧");
    }
	if (loop_index != -1)
	{
       // KeyFrame* old_kf = getKeyFrame(loop_index, loop_iris_flag);
        KeyFrame* old_kf = getKeyFrame(loop_index);
        old_iris_flag = old_kf->iris_flag;
        printf(" 当前第 %d 帧(数据库索引为: %d )和 %d 号智能体的第 %d 帧产生回环  \n", cur_kf->index,frame_index,old_iris_flag, loop_index);
       // cout<<"二次检验connection: "<<cur_kf->findConnection(old_kf)<<endl;
        if (cur_kf->findConnection(old_kf))
        {
            ...
            m_optimize_buf.lock();
            optimize_buf.push(frame_index);
            cout<<"经过二次检验,添加第 "<<frame_index<<" 帧至优化队列"<<endl;
           // optimize_buf.push(cur_kf->iris_flag);
...
/////////////////////////////////////
    ...
    if (SAVE_LOOP_PATH)
    {
        //ofstream loop_path_file("/home/wjj/catkin_workspace/src/VINS-Fusion/loop_result.csv", ios::app);
    ...
    if (SHOW_L_EDGE)
    {
        if (cur_kf->has_loop)
        {
            //printf("has loop \n");
////////////////////////////////By WJJ////////////////////////////////
      //      KeyFrame* connected_KF = getKeyFrame(cur_kf->loop_index,cur_kf->iris_flag);
...
void PoseGraph::kflist(KeyFrame* cur_kf)
{
     m_keyframelist.lock();
...
///////////////////////////////By WJJ/////////////////////////////
///KeyFrame retrieved by index and iris_flag(anent index)////
//KeyFrame* PoseGraph::getKeyFrame(int index, int iris_flag)
...
////////////cur_keyframe is keyftame in iris while frame_index is iris_index in database////
int PoseGraph::detectLoop(KeyFrame* keyframe, int frame_index)
{
    ...
    if (DEBUG_IMAGE)
    {
       ...
       // printf("第%d 个iris 的第%d个的索引图 \n",keyframe->iris_flag,frame_index);
    }
    TicToc tmp_t;
   ...
   // cout << "正在检验1号智能体第 " << keyframe->index <<" 帧,"<< "结果为: " << ret << endl;
   // cout << "正在检验1号智能体第 " << keyframe->index <<" 帧" << endl;
    TicToc t_add;
    //printf("在detectLoop添加关键帧");
    //keyframe->index = frame_index;
    //db.add(keyframe->brief_descriptors);

  //  cout << "database info: " << db << endl;
   // cout<<GREEN<<keyframe->iris_flag<<" 号智能体的第 "<<keyframe->index<<" 帧添加到数据库的索引为 "<<frame_index<<WHITE<<endl;
    ...
    if (ret.size() >= 1 &&ret[1].Score > 0.05)
        for (unsigned int i = 1; i < ret.size(); i++)
        {
            ...
            {          
                find_loop = true;
                int tmp_index = ret[i].Id;
          //      KeyFrame* ret_kf = getKeyFrame(ret.size());
           //     cout<<"检测回环帧索引为: "<<ret_kf->index<<endl;
                if (DEBUG_IMAGE && 0)
                {
                    auto it = image_pool.find(tmp_index);
                    cv::Mat tmp_image = (it->second).clone();
...
*/
    ...
    //     printf("找到回环");
  //      return min_index ,keyframe->iris_flag;
        return min_index ;
    }
    else
        return -1;
...
void PoseGraph::addKFIntoVoc(KeyFrame* keyframe,int frame_index)
{
    // put iris_2 KeyFrames into database
    keyframe->index = frame_index;
    cv::Mat compressed_image;
    if (DEBUG_IMAGE)
    {
        int feature_num = keyframe->keypoints.size();
       // printf("feature_num: %d",feature_num);
        cv::resize(keyframe->image, compressed_image, cv::Size(376, 240));
        putText(compressed_image, "feature_num:" + to_string(feature_num), cv::Point2f(10, 10), CV_FONT_HERSHEY_SIMPLEX, 0.4, cv::Scalar(255));
        image_pool[keyframe->index] = compressed_image;
       // printf("第%d 个iris 的第%d个的索引图 \n",keyframe->iris_flag,keyframe->index);
    }
    m_keyframelist.lock();
    db.add(keyframe->brief_descriptors);
   // kflist(keyframe);
   // cout<<YELLOW<<keyframe->iris_flag<<" 号智能体"<<"添加第"<<keyframe->index<<"帧关键帧至数据库"<<WHITE<<endl;
    //添加KeyFrame至关键帧链表
    keyframelist.push_back(keyframe);
    //printf("iris1+2关键帧列表大小为%d \n",keyframelist.size());
    m_keyframelist.unlock();
}

void PoseGraph::addKFIntoVocself(KeyFrame* keyframe,int frame_index)
{
    // put iris_2 KeyFrames into database
    keyframe->index = frame_index;
    cv::Mat compressed_image;
  ...
  //  m_keyframelistself.lock();
    db.add(keyframe->brief_descriptors);
   // cout<<GREEN<<keyframe->iris_flag<<" 号智能体"<<"添加第"<<global_index<<"帧关键帧至数据库索引为"<<keyframe->index<<WHITE<<endl;
    //添加KeyFrame至关键帧链表
    //keyframelist.push_back(keyframe);
    //printf("iris1+2关键帧列表大小为%d \n",keyframelist.size());
   // m_keyframelistself.unlock();
}
...
void PoseGraph::optimize4DoF()
{
    while(true)
    {
        ...
        {
            //printf("待优化的关键帧队列大小为%d \n", optimize_buf.size());
            cur_index = optimize_buf.front();
            first_looped_index = earliest_loop_index;
            optimize_buf.pop();
         //   cur_iris_flag = optimize_buf.front();
          //  optimize_buf.pop();
         //   printf("优化后的关键帧队列大小为%d \n", optimize_buf.size());
        }
        m_optimize_buf.unlock();
            if (cur_index != -1)
            {
            // printf("optimize pose graph \n");
                TicToc tmp_t;
                m_keyframelist.lock();
            //   KeyFrame* cur_kf = getKeyFrame(cur_index, cur_iris_flag);
                KeyFrame* cur_kf = getKeyFrame(cur_index);
         //       cur_iris_flag = cur_kf->iris_flag;
            //    printf("优化第%d个智能体的%d帧关键帧位姿图 \n",cur_iris_flag,cur_index);
